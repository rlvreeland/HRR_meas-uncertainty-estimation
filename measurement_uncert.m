classdef measurement_uncert
    % please not all variables inside of this object are the uncertainties
    % of each of these parameters. The fire model object holds the quantity
    ...
    properties
        ...
        % measurement. Each show the effect in mole fraction.
        O2_sens_flow = 5*10^(-6);
        ...
        % assuming a square distribution
        E_kJ_per_kg = 655;
        ...
        % below are calculated to be a reference for the uncertainty in the
        % measurement. These uncertainties are not used during the HRR
    ...
    methods
        ...
        function [obj_uncert] = ...
            ...
            % Create matching array with the uncertainties
            ...
            %%% Misc parameters for each species
            % Note these are place holder values with just uncertainty
            % assigned, no basis error is assumed
            % Create dl array
            misc_CO_dl = dlarray([0, 0, 0]); %[intrinsic, linearity, repeatability]
            misc_CO2_dl = dlarray([0, 0, 0]); %[intrinsic, linearity, repeatability]
            ...
            % Apply this operation for each HRR individually
            % Preallocate
            ...
            for i = 1:(length(obj_fire_model.x_CO_measure)+1)
                ...
                if isnan(conc_O2)
                    ...
                    %%% X_O2 Uncertainty Considering X_CO2
                    % Get partials as dl arrays
        ...
        function [obj_uncert] = ...
            ...
            % Complete assessment for each HRR individually
            % Preallocate
        ...
        function [obj_uncert, HRR_calc, ds_uncert_fraction] = ...
                uncert_HRR(obj_uncert, obj_fire_model, include_CO_CO2)
            % This function calculates the uncertainty in the HRR given the
            % data set.
            arguments
                obj_uncert 
                obj_fire_model 
            ...
            %%% Misc parameters for each species
            % Note these are place holder values with just uncertainty
            % assigned, assuming no bias error
            % Create dlarray
            misc_CO_dl = dlarray([0, 0, 0]); %[intrinsic, linearity, repeatability]
            misc_CO2_dl = dlarray([0, 0, 0]); %[intrinsic, linearity, repeatability]
            misc_O2_dl = dlarray([0, 0, 0, 0, 0, 0, 0, obj_fire_model.CO2_zero_offset_percent]);
            ...
            %%% Gas Constant including ambient conditions
            % Create dlarray
            gas_cons_dl = dlarray([obj_fire_model.R_m3_Pa_per_K_mol,...
                obj_fire_model.P_amb_Pa, obj_fire_model.Mw_air_g_per_mol, ...
            ...
            %%% Oxygen consumption calorimetery constants
            % Create dlarray
            ...
            for i = 1:(length(obj_fire_model.x_CO_measure))
                if isnan(obj_fire_model.x_O2_measure(i))
                    ...
                    %%% Extra Parameters for concentration calculation ...
                    % that have no assigned uncertainty.
                    % Make dlarray
                ...
                % Convert to regular arrays and calculate uncertainty
                % components^2
                ...
                obj_uncert.HRR_kW(i) = (sum(delta_part2_cal_gas, "all")+ ...
                 ...
                 % Calculate uncertainty fractions to save for later
                ...
                % Covert to expanded uncertainty 
