...
% This function calculates the HRR and the partial derivatives with respect
% to each variable. The CO and CO2 determines if CO and CO2 are included in
% the HRR calculation
...
%                    x_i_bottle_zero]
% misc_CO = [intrinsic, linearity, repeatability];
% misc_CO2 = [intrinsic, linearity, repeatability];
...
% conc_extra = given values of CO, CO2, O2, then the span range for CO,
...
% Oxygen consumption calorimtery constants
alpha = OCC_cons(1);
...
% Extra concentration parameters
...
%% Calculate concentration of gas species
...
% for x_O2 have to use the x_CO2_calc so that the uncertainty in CO2 
...
% this determines if CO and CO2 are used or not
...
else % Signifies that string contains 'true' for include_CO_CO2 
    % OCC: CO, CO2, O2
     phi = (x_O2_amb*(1-x_CO2_calc-x_CO_calc)-x_O2_calc*(1-x_CO2_amb))/...
        ((1-x_O2_calc-x_CO2_calc-x_CO_calc)*x_O2_amb);

    HRR = (E_kJ_per_kg*phi - (E_CO_kJ_per_kg-E_kJ_per_kg)*(1-phi)/2*...
        x_CO_calc/x_O2_calc)* m_e_kg_per_s /(1+phi*(alpha-1))*...
        Mw_O2_g_per_mol/Mw_air_g_per_mol*x_O2_amb;
end

%% Calculate the partial derivatives for each of the dlarrays used
